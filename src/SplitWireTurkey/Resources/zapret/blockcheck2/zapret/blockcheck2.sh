...
check_domain_prolog()
{
	...
	curl_test $1 $3 && {
		...
		# SplitWire-Turkey: bu satir daha once "true" olarak degistirilmisti (bypass'siz
		# test basarili olsa bile protokolu hep test etmeye zorlamak icin). alimali54/
		# zapret-win-turkey ile TAM BIREBIR ayni davranmak icin (kullanicinin acik talebi:
		# "argüman yakalama ve blockcheck2 mantığı ve configini tamamen zapret-win-turkey
		# ile aynı yapalım") asil zapret2 vendor satirina geri donduruldu -- artik
		# ENABLE_HTTP=0 (bkz. ask_params) sayesinde port 80 zaten hic test edilmiyor, o
		# yuzden bu satirin daha once cozdugu "port 80 only" sorunu zaten baska bir
		# yoldan (ENABLE_HTTP=0) cozulmus durumda ve bu ek degisikligin artik gerekcesi
		# kalmadi.
...
ask_params()
{
	...
	# SplitWire-Turkey: bu fonksiyonun ORİJİNAL (yukarıda kaldırılan) hâli tamamen
	# interaktifti -- vanilla upstream bol-van/zapret2, tıpkı alimali54/zapret-win-turkey'in
	# BİZİM gibi kendi bundled blockcheck2.sh'ında olduğu gibi. O proje ise blockcheck2.sh'ı
	# DOĞRUDAN KAYNAKTA değiştirip aşağıdaki "Auto-configure parameters for silent execution"
	# bloğuyla TÜM ask_params() gövdesini değiştirmiş (kullanıcının indirip incelediği resmi
	# release paketinden birebir alındı) -- kendi blog.sh'ları bizim blog-hidden.sh'ımız gibi
	# hiçbir env var (BATCH/SCANLEVEL/DOMAINS/IPVS) SET ETMİYOR, ihtiyaç yok çünkü zaten
	# blockcheck2.sh'ın kendisi silent moda göre yazılmış. EN ÖNEMLİ FARK: ENABLE_HTTP
	# varsayılanı "0" -- yani HTTP (port 80) HİÇ test edilmiyor, doğrudan HTTPS_TLS12'ye
	# geçiliyor. Bizim eski (vanilla) davranışımızda ENABLE_HTTP=1 idi, bu da blockcheck2'nin
	# önce TÜM 12 test dosyasını HTTP için tüketip (bazen dakikalarca sürüyor) SONRA HTTPS'e
	# geçmesine yol açıyordu -- kullanıcının "zapret-win-turkey'de argümanlar hemen
	# bulunuyordu, bizimkinde bulunmuyordu" şikayetinin GERÇEK kök nedeni buydu. Bu blok artık
	# BİREBİR aynı.
	DOMAINS=${DOMAINS:-"roblox.com"}
	...
	ENABLE_HTTPS_TLS13=${ENABLE_HTTPS_TLS13:-"0"}
