...
## Установка с помощью Setup файла (Рекомендуется) [![Download Setup](https://img.shields.io/badge/Download-Setup-blue?logo=windows)](https://github.com/cagritaskn/SplitWire-Turkey/releases/download/1.5.6/SplitWire-Turkey-Setup-Windows-1.5.6.exe)
- Скачайте пакет установки **[SplitWire-Turkey Setup](https://github.com/cagritaskn/SplitWire-Turkey/releases/download/1.5.6/SplitWire-Turkey-Setup-Windows-1.5.6.exe)** и выполните установку SplitWire-Turkey. (Если вы получите предупреждение SmartScreen "Windows защитил ваш персональный компьютер", нажмите "Подробнее", а затем нажмите "Выполнить в любом случае", информация о сканировании вирусов и этом предупреждении приведена ниже)
- Откройте приложение **SplitWire-Turkey**.
- Следуйте разделу **Руководства по использованию** для использования приложения.

## Использование с ZIP файлом (Не рекомендуется) [![Download ZIP](https://img.shields.io/badge/Download-ZIP-blue?logo=windows)](https://github.com/cagritaskn/SplitWire-Turkey/releases/download/1.5.6/SplitWire-Turkey-ZIP-Windows-1.5.6.zip)
- Скачайте **[SplitWire-Turkey ZIP](https://github.com/cagritaskn/SplitWire-Turkey/releases/download/1.5.6/SplitWire-Turkey-ZIP-Windows-1.5.6.zip)** файл и извлеките его в папку.
...
- **WS Стандартная Установка:** Устанавливает WireSock 3.4.8.1 вместе с ByeDPI-Drover; текстовые каналы маршрутизируются через ByeDPI, голосовые каналы — через WireSock. (Если включена опция "Туннелировать также браузеры", туннелирование также выполняется для интернет-браузеров)

- **WS Альтернативная Установка:** Устанавливает только автономный WireSock 3.4.8.1 (без ByeDPI и Drover); весь трафик туннелируется напрямую через WireSock. (Если включена опция "Туннелировать также браузеры", туннелирование также выполняется для интернет-браузеров)
...
## Использование Страницы Zapret2

**Примечание:** Установки в этом разделе работают на уровне всей системы. Хотя они не вызывают потери скорости, они могут привести к проблемам с подключением на некоторых веб-сайтах и приложениях. После выполнения этих установок соответствующий метод будет автоматически запускаться при каждой перезагрузке системы.

- **Автоматическая Установка Zapret2:** Идеальные параметры для вашей системы и интернет-провайдера находятся с помощью программного обеспечения поиска стратегий blockcheck от Zapret2, и обход DPI достигается путем установки Zapret2 с этими параметрами.
...
> Эти времена являются приблизительными. Они могут варьироваться в зависимости от вашей системы и политик проверки пакетов вашего интернет-провайдера.

- **Предустановка:** Выбирает один из предопределенных параметров для Zapret2. Включены предустановки для Türk Telekom, Superonline, Vodafone, Türksat Kablonet, Telekom Mobil, Turkcell Mobil и Vodafone Mobil; они также автоматически пробуются перед полным сканированием в Автоматической Установке. (Спасибо Bal Porsuğu за предустановки)

- **Редактировать предустановку:** Открывает текстовое поле, которое позволяет вам точно настроить или изменить выбранную предустановку. После внесения изменений в это поле вы можете использовать кнопки ниже для установки или одноразового запуска с параметрами в поле.

- **Использовать список исключений:** Использует список доменов, к которым Zapret2 не будет применять обход DPI (не будет их трогать). При отключении список временно очищается (перезапуск Zapret2 не требуется, изменение применяется автоматически); при повторном включении список возвращается.

- **Редактировать список исключений:** Открывает текстовое поле, где можно редактировать домены в списке исключений. После редактирования нажмите Сохранить, чтобы сохранить изменения.

- **Установить службу предустановки:** Устанавливает службу Zapret2 с выбранной предустановкой (или отредактированной версией, если вы внесли изменения).

- **Предустановка одноразово:** Запускает Zapret2 один раз с выбранной предустановкой (или отредактированной версией, если вы внесли изменения). Zapret2 перестает работать, когда вы закрываете открытое окно консоли.

- **Удалить Zapret2:** Удаляет Zapret2.
...
- **Применять настройки DNS и DoH при каждой установке:** Системная настройка DNS автоматически применяется во всех установках методов обхода, которые могут быть выполнены в SplitWire-Turkey. Какой метод используется (DnsCrypt-Proxy или Google DNS/Quad9 с DoH) определяется переключателем **Метод DNS** прямо под этой опцией. Вы можете полностью отключить автоматическую настройку DNS и DoH (а значит и установку DnsCrypt-Proxy), выключив этот переключатель.

- **Метод DNS (DnsCrypt-Proxy / Windows DoH):** Когда включен (и переключатель DNS выше тоже включен), DnsCrypt-Proxy (зашифрованный DNS) автоматически устанавливается и запускается при каждой установке/одноразовом запуске/blockcheck; ваш DNS-сервер указывает на 127.0.0.1 и ::1 (предотвращая утечки в открытом виде). Если DnsCrypt-Proxy уже установлен, он не переустанавливается. Если этот переключатель выключен, вместо этого используется нативный Windows DoH (Google DNS и Quad9). Оба метода никогда не используются одновременно.
...
> [!NOTE]
> Вы также можете обратить внимание на наш новый проект **[SplitCord-Turkey](https://github.com/cagritaskn/SplitCord-Turkey)**.
...
- Если у вас по-прежнему возникают проблемы с клиентом Discord, вы можете попробовать альтернативный проект **[SplitCord-Turkey](https://github.com/cagritaskn/SplitCord-Turkey)**.
...
- **[zapret2](https://github.com/bol-van/zapret2)** от **[bol-van](https://github.com/bol-van)**
...
Находит идеальные параметры для вашей системы и интернет-провайдера с использованием технологии blockcheck2 и обеспечивает установку службы, объединяя эти параметры с вашими предпочтениями. Готовые настройки для известных турецких интернет-провайдеров, таких как Türk Telekom, Superonline и Vodafone, сначала пробуются автоматически; если одна из них работает, полное сканирование вообще не требуется. Выбор скорости сканирования регулирует, насколько простым или глубоким будет сканирование параметров.

- Установка и Одноразовый Запуск Zapret2 с Пресетами
Служба Zapret2 устанавливается или запускается один раз с предопределенными готовыми настройками (или отредактированными версиями, если вы внесли изменения).
...
- Метод DNS (DnsCrypt-Proxy / Windows DoH) и Вернуть Настройки DNS и DoH
Перед любой установкой, которую вы делаете в SplitWire-Turkey, все службы очищаются для чистой установки, затем в зависимости от переключателя **Метод DNS** на вкладке **Дополнительно** применяется один из двух методов (никогда оба одновременно): когда переключатель включен, устанавливается DnsCrypt-Proxy, и ваш DNS указывает на 127.0.0.1 и ::1 (зашифрованный DNS); когда переключатель выключен, активируется поддерживаемый Windows 11 DoH, и выполняется назначение IPv4 и IPv6 DNS (Google первичный и Quad9 вторичный DNS; DoH не поддерживается для Windows 10 и более ранних версий). Кнопка Вернуть настройки DNS и DoH отменяет все эти настройки независимо от использованного метода, возвращая назначения DNS к Автоматически (DHCP) и выключая DoH.
